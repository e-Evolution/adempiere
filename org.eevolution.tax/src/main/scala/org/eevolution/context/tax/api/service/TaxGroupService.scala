...
object TaxGroupService {

	type TaxGroupService = Has[Service]
	...
	def live: ZLayer[TaxGroupRepository.TaxGroupRepository, Nothing, Has[Service]]
	= ZLayer.fromService[TaxGroupRepository.Service, Service] { taxGroupRepository => TaxGroupServiceLive(taxGroupRepository) }

	def getByDescription(description: Option[String]): RIO[TaxGroupService,List[TaxGroup]]
	= RIO.accessM(_.get.getByDescription(description))

	def getByName(name: String): RIO[TaxGroupService, Option[TaxGroup]]
	= RIO.accessM(_.get.getByName(name))

	def getByValue(value: String): RIO[TaxGroupService, Option[TaxGroup]]
	= RIO.accessM(_.get.getByValue(value))

	def getByTaxGroupId(taxGroupId: Id): RIO[TaxGroupService, Option[TaxGroup]]
	= RIO.accessM(_.get.getByTaxGroupId(taxGroupId))

	def getAll: RIO[TaxGroupService,List[TaxGroup]]
	= RIO.accessM(_.get.getAll)

	def getAll(clientId: TableDirect): RIO[TaxGroupService,List[TaxGroup]]
