...
object TaxCategoryService {

	type TaxCategoryService = Has[Service]
	...
	def live: ZLayer[TaxCategoryRepository.TaxCategoryRepository, Nothing, Has[Service]]
	= ZLayer.fromService[TaxCategoryRepository.Service, Service] { taxCategoryRepository => TaxCategoryServiceLive(taxCategoryRepository) }

	def getByDescription(description: Option[String]): RIO[TaxCategoryService,List[TaxCategory]]
	= RIO.accessM(_.get.getByDescription(description))

	def getByName(name: String): RIO[TaxCategoryService, Option[TaxCategory]]
	= RIO.accessM(_.get.getByName(name))

	def getByTaxCategoryId(taxCategoryId: Id): RIO[TaxCategoryService, Option[TaxCategory]]
	= RIO.accessM(_.get.getByTaxCategoryId(taxCategoryId))

	def getAll: RIO[TaxCategoryService,List[TaxCategory]]
	= RIO.accessM(_.get.getAll)

	def getAll(clientId: TableDirect): RIO[TaxCategoryService,List[TaxCategory]]
