...
object TaxDeclarationService {

	type TaxDeclarationService = Has[Service]
	...
	def live: ZLayer[TaxDeclarationRepository.TaxDeclarationRepository, Nothing, Has[Service]]
	= ZLayer.fromService[TaxDeclarationRepository.Service, Service] { taxDeclarationRepository => TaxDeclarationServiceLive(taxDeclarationRepository) }

	def getByDescription(description: Option[String]): RIO[TaxDeclarationService,List[TaxDeclaration]]
	= RIO.accessM(_.get.getByDescription(description))

	def getByName(name: String): RIO[TaxDeclarationService, Option[TaxDeclaration]]
	= RIO.accessM(_.get.getByName(name))

	def getByTaxDeclarationId(taxDeclarationId: Id): RIO[TaxDeclarationService, Option[TaxDeclaration]]
	= RIO.accessM(_.get.getByTaxDeclarationId(taxDeclarationId))

	def getAll: RIO[TaxDeclarationService,List[TaxDeclaration]]
	= RIO.accessM(_.get.getAll)

	def getAll(clientId: TableDirect): RIO[TaxDeclarationService,List[TaxDeclaration]]
