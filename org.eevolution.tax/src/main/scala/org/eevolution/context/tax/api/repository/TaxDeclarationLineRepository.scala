...
object TaxDeclarationLineRepository {

	type TaxDeclarationLineRepository = Has[TaxDeclarationLineRepository.Service]
	...
	def getByDescription(description: Option[String]): RIO[TaxDeclarationLineRepository,List[TaxDeclarationLine]]
	= RIO.accessM(_.get.getByDescription(description))

	def getByTaxDeclarationId(taxDeclarationId: TableDirect): RIO[TaxDeclarationLineRepository,List[TaxDeclarationLine]]
	= RIO.accessM(_.get.getByTaxDeclarationId(taxDeclarationId))

	def getByTaxDeclarationLineId(taxDeclarationLineId: Id): RIO[TaxDeclarationLineRepository, Option[TaxDeclarationLine]]
	= RIO.accessM(_.get.getByTaxDeclarationLineId(taxDeclarationLineId))

	def getAll: RIO[TaxDeclarationLineRepository,List[TaxDeclarationLine]]
	= RIO.accessM(_.get.getAll)

	def getAll(clientId: TableDirect): RIO[TaxDeclarationLineRepository,List[TaxDeclarationLine]]
