...
object TaxTypeRepository {

	type TaxTypeRepository = Has[TaxTypeRepository.Service]
	...
	def getByDescription(description: Option[String]): RIO[TaxTypeRepository,List[TaxType]]
	= RIO.accessM(_.get.getByDescription(description))

	def getByName(name: String): RIO[TaxTypeRepository, Option[TaxType]]
	= RIO.accessM(_.get.getByName(name))

	def getByValue(value: String): RIO[TaxTypeRepository, Option[TaxType]]
	= RIO.accessM(_.get.getByValue(value))

	def getByTaxTypeId(taxTypeId: Id): RIO[TaxTypeRepository, Option[TaxType]]
	= RIO.accessM(_.get.getByTaxTypeId(taxTypeId))

	def getAll: RIO[TaxTypeRepository,List[TaxType]]
	= RIO.accessM(_.get.getAll)

	def getAll(clientId: TableDirect): RIO[TaxTypeRepository,List[TaxType]]
