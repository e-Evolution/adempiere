...
object ContractRepository {
  ...
  type ContractRepository = Has[Service]

  trait Service {
    def getById(id: Id): ZIO[Context, Throwable, Contract]
  }

  def live: ZLayer[Has[Context], Nothing, Has[Service]] = ZLayer.fromService[Context, Service] { contextService => ContractRepositoryLive(contextService)}
}

case class ContractRepositoryLive(contextService: Context) extends Service {
  override def getById(id: Id): ZIO[Context, Throwable, Contract] = ???
}
