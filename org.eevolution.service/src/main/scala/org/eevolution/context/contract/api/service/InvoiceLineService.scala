...
object InvoiceLineService {

  type InvoiceLineService =  Has[Service]
  ...
  def live: ZLayer[Has[Context.Service] with Has[InvoiceLineRepository.Service], Nothing, Has[Service]] = ZLayer.fromServices[Context.Service, InvoiceLineRepository.Service, Service] {
   (contextService, invoiceLineRepository) => InvoiceLineServiceLive (contextService, invoiceLineRepository)
  }
}
