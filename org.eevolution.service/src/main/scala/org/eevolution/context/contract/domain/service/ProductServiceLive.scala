...
case class ProductServiceLive(contextService: Context.Service, productRepository: ProductRepository.Service)  extends Service {
  override def getById(id: Id): ZIO[Context, Throwable, Product] = productRepository.getById(id)

  override def getByServiceTypeId(serviceTypeId: Id): ZIO[Context, Throwable, Product] = productRepository.getByServiceTypeId(serviceTypeId)
}