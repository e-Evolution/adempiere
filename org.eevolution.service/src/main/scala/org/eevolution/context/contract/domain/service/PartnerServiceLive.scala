...
import org.eevolution.context.contract.api.service.PartnerService.Service
...
case class PartnerServiceLive(contextService : Context.Service, partnerRepositoryService : PartnerRepository.Service) extends Service {
  override def getById(id: Id): ZIO[Context, Throwable, Partner] = partnerRepositoryService.getById(id)
}