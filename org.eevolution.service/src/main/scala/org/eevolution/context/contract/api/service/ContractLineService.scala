...
object ContractLineService {
  ...
  trait Service {
    ...
    def getByContract(contractId: Id): ZIO[Context, Throwable, List[ContractLine]]

    def query(whereClause: String, parameters: List[Object]): ZIO[Context, Throwable, List[ContractLine]]
  }

  def live: ZLayer[Has[Context.Service] with Has[ContractLineRepository.Service], Nothing, Has[Service]] = ZLayer.fromServices[Context.Service, ContractLineRepository.Service , Service] {
    (contextService , contractLineRepository) => ContractLineServiceLive(contextService, contractLineRepository)
  }
}
...
case class ContractLineServiceLive(contextService: Context.Service, contractLineRepository: ContractLineRepository.Service) extends Service {
  ...
  override def getByContract(contractId: Id): ZIO[Context, Throwable, List[ContractLine]] = ???
