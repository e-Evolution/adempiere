...
import org.eevolution.context.contract.api.service.ContractService.Service
...
case class ContractServiceLive(contextService : Context.Service, contractRepository : ContractRepository.Service) extends Service {
  override def getById(id: Id): ZIO[Context, Throwable, Contract] = contractRepository.getById(id)
}
