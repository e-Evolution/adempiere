...
object AgreementTypeService {

	type AgreementTypeService = Has[Service]
	...
	def live: ZLayer[AgreementTypeRepository.AgreementTypeRepository, Nothing, Has[Service]]
	= ZLayer.fromService[AgreementTypeRepository.Service, Service] { agreementTypeRepository => AgreementTypeServiceLive(agreementTypeRepository) }

	def getByAgreementTypeId(agreementTypeId: Id): RIO[AgreementTypeService, Option[AgreementType]]
	= RIO.accessM(_.get.getByAgreementTypeId(agreementTypeId))

	def getByDescription(description: Option[String]): RIO[AgreementTypeService,List[AgreementType]]
	= RIO.accessM(_.get.getByDescription(description))

	def getByName(name: String): RIO[AgreementTypeService, Option[AgreementType]]
	= RIO.accessM(_.get.getByName(name))

	def getByValue(value: Option[String]): RIO[AgreementTypeService, Option[AgreementType]]
	= RIO.accessM(_.get.getByValue(value))

	def getAll: RIO[AgreementTypeService,List[AgreementType]]
	= RIO.accessM(_.get.getAll)

	def getAll(clientId: TableDirect): RIO[AgreementTypeService,List[AgreementType]]
