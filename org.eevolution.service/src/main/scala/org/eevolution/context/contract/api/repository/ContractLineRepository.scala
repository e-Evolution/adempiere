...
object ContractLineRepository {
	...
	type ContractLineRepository = Has[ContractLineRepository.Service]
	...
	def getByContractId(contractId: TableDirect): RIO[ContractLineRepository,List[ContractLine]]
	= RIO.accessM(_.get.getByContractId(contractId))

	def getByContractLineId(contractLineId: Id): RIO[ContractLineRepository, Option[ContractLine]]
	= RIO.accessM(_.get.getByContractLineId(contractLineId))

	def getByDescription(description: Option[String]): RIO[ContractLineRepository,List[ContractLine]]
	= RIO.accessM(_.get.getByDescription(description))

	def getByLineNetAmt(lineNetAmt: Amount): RIO[ContractLineRepository,List[ContractLine]]
	= RIO.accessM(_.get.getByLineNetAmt(lineNetAmt))

	def getByLine(line: Number): RIO[ContractLineRepository,List[ContractLine]]
	= RIO.accessM(_.get.getByLine(line))

	def getAll: RIO[ContractLineRepository,List[ContractLine]]
	= RIO.accessM(_.get.getAll)

	def getAll(clientId: TableDirect): RIO[ContractLineRepository,List[ContractLine]]
