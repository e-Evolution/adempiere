...
/**
  ...
  * @param trxName
  */
class MSContract(ctx: Properties, contractId: Int, trxName: String)
  extends X_S_Contract(ctx: Properties, contractId: Int, trxName: String) with DocAction {

  def getLines() : util.List[Nothing] = {
    ...
    return new Query(getCtx , I_S_ContractLine.Table_Name , whereClause , get_TrxName())
      .setClient_ID()
      .setParameters(getS_Contract_ID.asInstanceOf[Integer])
      .list()
  }

  def apply(ctx: Properties, rs: ResultSet, trxName: String) = {
    new X_S_Contract(ctx: Properties, rs: ResultSet, trxName: String)
  }
